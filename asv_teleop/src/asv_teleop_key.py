...
class AckermannDriveKeyop:
	def __init__(self): #, args):
		# if len(args) == 1:
		# 	rospy.loginfo("Setting up max linear velocity")
		# 	max_speed = float(args[0])
		# elif len(args) == 2:
		# 	rospy.loginfo("Setting up max linear velocity and max steering angle")
		# 	max_speed = float(args[0])
		# 	max_steering_angle = float(args[1])
		# else:
		max_speed = 0.075
		max_steering_angle = 80 / degree_per_rad 
		...
		# Initialize the speed commands
		for key in key_bindings:
			key_bindings[key] = (key_bindings[key][0] * float(max_speed) / 5, key_bindings[key][1] * float(max_steering_angle) / 5)
	...
	def pub_callback(self, event):
		...
		ackermann_cmd_msg.header.frame_id = self.frame_id
